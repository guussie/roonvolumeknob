...
// ====== USER CONFIGURATION ======
// Set your device configuration here
const DEFAULT_DEVICE_MAC_ADDRESS = "F0:19:88:40:85:22"; // Bluetooth MAC address
const DEFAULT_STARTUP_VOLUME = 5; // Initial volume level (0-100)
const DEFAULT_DEVICE_NAME = "VOL20"; // Bluetooth device name
const DEFAULT_EXTENSION_NAME = "Bluetooth Volume Control"; // Roon extension display name
const DEFAULT_ZONE_NAME = "Living Room"; // Default zone name for this instance
const DEFAULT_HOMEBRIDGE_URL = "http://192.168.68.206:51828"; // Homebridge server URL
const DEFAULT_ACCESSORY_ID_PREFIX = "vol20_"; // Prefix for accessory ID in Homebridge
// =============================== 
...
let currentVolume = DEFAULT_STARTUP_VOLUME;
let initialVolumeSet = false;
let webhookUrl = null;
let bluetoothConfig = {
    deviceName: DEFAULT_DEVICE_NAME,
    macAddress: DEFAULT_DEVICE_MAC_ADDRESS,
    startupVolume: DEFAULT_STARTUP_VOLUME
};
...
let homebridgeConfig = {
    enabled: false,
    url: DEFAULT_HOMEBRIDGE_URL,
    accessoryId: `${DEFAULT_ACCESSORY_ID_PREFIX}${DEFAULT_ZONE_NAME.toLowerCase().replace(/\s+/g, '_')}_battery`,
    deviceName: `${DEFAULT_DEVICE_NAME}-${DEFAULT_ZONE_NAME.replace(/\s+/g, '')}` // Use the device name and zone in the identifier
};
...
function loadConfig() {
    try {
        if (fs.existsSync(CONFIG_FILE)) {
            ...
            // Load selected zone if it exists
            if (config.selectedZone) {
                selectedZone = config.selectedZone;
                console.log("Loaded saved zone:", selectedZone);
            }
            
            // Load bluetooth config if it exists
            if (config.bluetoothConfig) {
                bluetoothConfig = {
                    ...bluetoothConfig, // Keep defaults
                    ...config.bluetoothConfig // Override with saved values
                };
                console.log("Loaded Bluetooth configuration:", bluetoothConfig);
            }
        } else {
...
function findVOL20Device() {
    return new Promise((resolve, reject) => {
        ...
        exec(`bluetoothctl devices | grep -i "${bluetoothConfig.deviceName}"`, (error, stdout, stderr) => {
            console.log(`Checking bluetooth devices for ${bluetoothConfig.deviceName}...`);
            
            if (!error && stdout.trim()) {
                const btDeviceMatch = stdout.match(/([0-9A-F:]{17})\s+${bluetoothConfig.deviceName}/i);
                if (btDeviceMatch) {
                    const btAddress = btDeviceMatch[1];
                    console.log(`Found ${bluetoothConfig.deviceName} with address: ${btAddress}`);
                    
                    // Check if the device is connected
                    exec(`bluetoothctl info ${btAddress} | grep "Connected: yes"`, (err, out) => {
                        if (!err && out.includes("Connected: yes")) {
                            console.log(`${bluetoothConfig.deviceName} is connected`);
                            ...
                            exec('ls -l /dev/input/event*', (err2, out2) => {
                                if (!err2) {
                                    ...
                                    if (eventDevs && eventDevs.length > 0) {
                                        console.log(`Found ${eventDevs.length} event devices. Checking each one for ${bluetoothConfig.deviceName}...`);
                                        
                                        // Check each event device sequentially
                                        findVOL20EventDevice(eventDevs, 0)
                                            .then(eventDevice => {
                                                if (eventDevice) {
                                                    console.log(`Found ${bluetoothConfig.deviceName} at ${eventDevice}`);
                                                    resolve(`/dev/input/${eventDevice}`);
                                                } else {
                                                    console.log("Could not find ${bluetoothConfig.deviceName} among event devices. Using fallback.");
                                                    resolve('/dev/input/event0');
                                                }
                                            })
                                            .catch(e => {
                                                console.error(`Error finding ${bluetoothConfig.deviceName} event device:`, e);
                                                resolve('/dev/input/event0');
                                            });
                                        return;
                                    }
                            ...
                            console.log(`${bluetoothConfig.deviceName} is not connected. Please check your Bluetooth connection.`);
                            // Fallback to a default device
                            resolve('/dev/input/event0');
                        }
                    });
                } else {
                    console.log(`${bluetoothConfig.deviceName} device not found in bluetoothctl devices list`);
                    resolve('/dev/input/event0');
                }
            } else {
                console.log(`Error listing Bluetooth devices or ${bluetoothConfig.deviceName} not found`);
                resolve('/dev/input/event0');
            }
        });
    });
...
function findVOL20EventDevice(eventDevs, index) {
    return new Promise((resolve, reject) => {
        ...
        process.on('close', (code) => {
            // Check if the output contains device name
            if (output.includes(bluetoothConfig.deviceName)) {
                console.log(`Found ${bluetoothConfig.deviceName} in ${eventDevice}`);
                resolve(eventDevice);
            } else {
                // Try with a direct grep approach as a backup
                exec(`sudo evtest ${devicePath} | grep -i "${bluetoothConfig.deviceName}"`, { timeout: 1000 }, (err, stdout) => {
                    if (!err && stdout.includes(bluetoothConfig.deviceName)) {
                        console.log(`Found ${bluetoothConfig.deviceName} in ${eventDevice} with grep`);
                        resolve(eventDevice);
                    } else {
                        // Check next device
...
// Function to generate extension display name based on zone
    ...
    if (outputs && outputs.length > 0) {
        ...
        if (selectedOutput.volume) {
            ...
            if (!initialVolumeSet && transport) {
                console.log(`Setting initial volume to ${bluetoothConfig.startupVolume}`);
                transport.change_volume(selectedOutput.output_id, "absolute", bluetoothConfig.startupVolume, function(error) {
                    if (error) {
    ...
    const html = `
        ...
        <body>
            ...
            <div class="status">
                ...
                <p>Device: ${bluetoothConfig.deviceName} (${bluetoothConfig.macAddress})</p>
            ...
            <div class="nav-links">
                ...
                <a href="/device-config">Device Config</a>
                <a href="/restart-app" style="color: #d9534f;">Restart App</a>
            </div>
...
// Add a new route for device configuration
app.get('/device-config', (req, res) => {
    // Get the current selected zone name or the default
    const currentZoneName = (selectedZone && zoneList[selectedZone]) ? 
                            zoneList[selectedZone].display_name : 
                            DEFAULT_ZONE_NAME;
    ...
    const html = `
        ...
        <body>
            ...
            <div class="current">
                ...
                <p><strong>Device Name:</strong> ${bluetoothConfig.deviceName}</p>
                <p><strong>MAC Address:</strong> ${bluetoothConfig.macAddress}</p>
                <p><strong>Startup Volume:</strong> ${bluetoothConfig.startupVolume}</p>
                <p><strong>Extension Name:</strong> ${getExtensionDisplayName()}</p>
                <p><strong>Default Zone:</strong> ${DEFAULT_ZONE_NAME}</p>
                <p><strong>Current Zone:</strong> ${currentZoneName}</p>
            </div>
            
            <div class="form">
                <form action="/save-device-config" method="post">
                    <div class="section">
                        <h2>Bluetooth Device Settings</h2>
                        <label for="deviceName">Device Name:</label>
                        <input type="text" id="deviceName" name="deviceName" value="${bluetoothConfig.deviceName}">
                        
                        <label for="macAddress">MAC Address:</label>
                        <input type="text" id="macAddress" name="macAddress" value="${bluetoothConfig.macAddress}" 
                               placeholder="Format: XX:XX:XX:XX:XX:XX">
                        
                        <label for="startupVolume">Startup Volume (0-100):</label>
                        <input type="number" id="startupVolume" name="startupVolume" 
                               value="${bluetoothConfig.startupVolume}" min="0" max="100">
                    </div>
                    
                    <div class="section">
                        <h2>Extension Settings</h2>
                        <label for="extensionName">Extension Base Name:</label>
                        <input type="text" id="extensionName" name="extensionName" value="${DEFAULT_EXTENSION_NAME}">
                        
                        <label for="defaultZone">Default Zone Name:</label>
                        <input type="text" id="defaultZone" name="defaultZone" value="${DEFAULT_ZONE_NAME}">
                        <p class="note" style="font-size: 0.9em; color: #666;">
                            Note: This will be used in the extension name and Homebridge configuration when no zone is selected.
                        </p>
                    </div>
...
app.post('/save-device-config', (req, res) => {
    ...
    // Update global constants (these will be overwritten on restart, but useful for current session)
    global.DEFAULT_EXTENSION_NAME = extensionName;
    global.DEFAULT_ZONE_NAME = defaultZone;
...
// Add a route to restart the Bluetooth monitoring
app.get('/restart-bluetooth', (req, res) => {
    // First kill any existing evtest processes
    exec('sudo killall evtest', (error) => {
        if (error) {
            console.log("No evtest processes to kill or error:", error);
        } else {
            console.log("Killed existing evtest processes");
        }
        ...
        res.send(`
            <html><body>
                <h2>Bluetooth Monitor Restarted</h2>
                <p>The Bluetooth monitoring process has been restarted.</p>
...
// Add a route to restart the entire application
app.get('/restart-app', (req, res) => {
    res.send(`
        <html><body>
            <h2>Restarting Application</h2>
            <p>The application will restart in 3 seconds...</p>
            <script>
                setTimeout(function() {
    ...
    // Log the restart
    console.log("Application restart requested through web interface");
    
    // Wait a moment to let the response be sent, then restart
    setTimeout(() => {
        console.log("Restarting application...");
        process.exit(0); // The process will be restarted by a process manager like PM2 or systemd
    }, 2000);
});
...
// Helper function to update Homebridge accessory ID based on zone
...
async function getDeviceBatteryInfo() {
    try {
        console.log(`Checking battery for device ${bluetoothConfig.macAddress}...`);
        
        // Use bluetoothctl info to get device information including battery
        const output = await new Promise((resolve, reject) => {
            exec(`bluetoothctl info ${bluetoothConfig.macAddress}`, (err, stdout) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(stdout);
...
async function ensureDeviceConnected() {
    console.log(`Checking connection to ${bluetoothConfig.deviceName} (${bluetoothConfig.macAddress})...`);
    
    try {
        // Check if device is connected
        const output = await new Promise((resolve, reject) => {
            exec(`bluetoothctl info ${bluetoothConfig.macAddress} | grep "Connected: yes"`, (err, stdout) => {
                if (err && err.code !== 1) {
                    reject(err);
                } else {
                    resolve(stdout);
        ...
        if (output && output.includes("Connected: yes")) {
            console.log(`${bluetoothConfig.deviceName} is already connected`);
            return true;
        }
        
        // Try to connect
        console.log(`${bluetoothConfig.deviceName} is not connected, attempting to connect...`);
        const connectOutput = await new Promise((resolve, reject) => {
            exec(`bluetoothctl connect ${bluetoothConfig.macAddress}`, (err, stdout) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(stdout);
        ...
        if (connectOutput.includes("Connection successful")) {
            console.log(`Successfully connected to ${bluetoothConfig.deviceName}`);
            return true;
        } else {
            console.log(`Failed to connect to ${bluetoothConfig.deviceName}`);
            return false;
        }
    } catch (err) {
        console.error(`Error connecting to ${bluetoothConfig.deviceName}:`, err);
        return false;
    }
}
...
setInterval(async () => {
    const connected = await ensureDeviceConnected();
    if (connected) {
        console.log(`${bluetoothConfig.deviceName} connection verified`);
    } else {
        console.log(`${bluetoothConfig.deviceName} connection check failed, will retry later`);
    }
}, 60000); // Check every minute

// Function to load extension configuration
function loadExtensionConfig() {
    try {
        const configFile = path.join(__dirname, 'extension-config.json');
        if (fs.existsSync(configFile)) {
            const configData = fs.readFileSync(configFile, 'utf8');
            const config = JSON.parse(configData);
            
            if (config.extensionName) {
                global.DEFAULT_EXTENSION_NAME = config.extensionName;
                console.log("Loaded extension name:", DEFAULT_EXTENSION_NAME);
            }
            
            if (config.defaultZone) {
                global.DEFAULT_ZONE_NAME = config.defaultZone;
                console.log("Loaded default zone:", DEFAULT_ZONE_NAME);
            }
        } else {
            console.log("No extension configuration found, using defaults");
        }
    } catch (err) {
        console.error("Error loading extension configuration:", err);
    }
}
